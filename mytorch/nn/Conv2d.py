...
class Conv2d_stride1():
    ...
    def forward(self, A):
        ...
        # 初始化输出Z
        Z = np.zeros((batch_size, self.out_channels,
    ...
    def backward(self, dLdZ):
        ...
        for k in range(self.out_channels):
            for i in range(output_height):
                for j in range(output_width):
                    self.dLdW[k, :, :, :] += np.sum(dLdZ[:, k, i, j][:, None, None, None]
                                                    * self.A[:, :, i:i+self.kernel_size, j:j+self.kernel_size], axis=0)
...
class Conv2d_padding():
    ...
    def backward(self, output_gradient):
        ...
        # 首先处理下采样层的梯度
        downsampled_output_gradient = self.downsample2d.backward(output_gradient)

        # 将梯度传递给步长为1的卷积层
        inputLossGradient = self.conv2d_stride1.backward(downsampled_output_gradient)

        if self.pad > 0:
            inputLossGradient = inputLossGradient[:, :, self.pad:-self.pad, self.pad:-self.pad]
